...
namespace impl // {{{ some command generator helpers
{
    ...
    template <typename TheTerminal>
    ApplyResult setOrRequestDynamicColor(Sequence const& _seq, Screen<TheTerminal>& _screen, DynamicColorName _name)
    {
    ...
    template <typename TheTerminal>
    ApplyResult setAllFont(Sequence const& _seq, Screen<TheTerminal>& _screen)
    {
        // [read]  OSC 60 ST
    ...
    template <typename TheTerminal>
    ApplyResult setFont(Sequence const& _seq, Screen<TheTerminal>& _screen)
    {
        auto const& params = _seq.intermediateCharacters();
        auto const splits = crispy::split(params, ';');
    ...
    template <typename TheTerminal>
    ApplyResult clipboard(Sequence const& _seq, Screen<TheTerminal>& _screen)
    {
    ...
    template <typename TheTerminal>
    ApplyResult CAPTURE(Sequence const& _seq, Screen<TheTerminal>& _screen)
    {
        // CSI Mode ; [; Count] t
        //
    ...
    template <typename TheTerminal>
    ApplyResult WINDOWMANIP(Sequence const& _seq, Screen<TheTerminal>& _screen)
    {
...
template <typename TheTerminal>
Sequencer<TheTerminal>::Sequencer(TheTerminal& _terminal,
...
unique_ptr<ParserExtension> Sequencer<TheTerminal>::hookSTP(Sequence const& /*_seq*/)
{
    return make_unique<SimpleStringCollector>([this](string_view const& _data) {
        screen().eventListener().setTerminalProfile(unicode::convert_to<char>(_data));
    });
}
...
ApplyResult Sequencer<TheTerminal>::apply(FunctionDefinition const& _function, Sequence const& _seq)
{
    ...
    {
    // C0
    case BEL: screen().eventListener().bell(); break;
    ...
    case REP:
        if (precedingGraphicCharacter_)
        {
            ...
            for (size_t i = 0; i < effectiveCount; i++)
                screen().writeText(precedingGraphicCharacter_);
        }
        break;
    case RM: {
        ApplyResult r = ApplyResult::Ok;
    ...
    case WINMANIP: return impl::WINDOWMANIP(_seq, screen());
    ...
    case CAPTURE: return impl::CAPTURE(_seq, screen());
    case COLORFG:
    ...
    case COLORCURSOR: return impl::setOrRequestDynamicColor(_seq, screen(), DynamicColorName::TextCursorColor);
    case COLORMOUSEFG:
    ...
    case SETFONT: return impl::setFont(_seq, screen());
    case SETFONTALL: return impl::setAllFont(_seq, screen());
    case CLIPBOARD: return impl::clipboard(_seq, screen());
